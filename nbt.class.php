 ...
 * 
 * @author  Justin Martin <frozenfire@thefrozenfire.com>, Stefan Kent (skent@twosphere.com.au)
 * @version 1.3
 *
 * SK: removed GMP dependancy
 *
 * Dependencies:
 *  PHP 5.3+
 */
...
class NBT {
	...
	public function loadFile_nbt($filename) {
	...
	protected function readType($fp, $tagType) {
		switch($tagType) {
			...
			case self::TAG_LONG: // Signed long (64 bit, big endian)
				...
				$value = $secondHalf + ($firstHalf * "4294967296");
				if($value >= pow(2, 63)) $value = $value - pow(2, 64);
				return $value;
	...
	protected function writeType($fp, $tagType, $value) {
		switch($tagType) {
			...
			case self::TAG_INT: // Signed integer (32 bit, big endian)
				...
				return is_int(fwrite($fp, pack("N", $value)));
			case self::TAG_LONG: // Signed long (64 bit, big endian)
				$secondHalf = $value % 2147483647;
				$firstHalf = ($value-$secondHalf);
				return is_int(fwrite($fp, pack("N", (int)$firstHalf))) && is_int(fwrite($fp, pack("N", (int)$secondHalf)));
			case self::TAG_FLOAT: // Floating point value (32 bit, big endian, IEEE 754-2008)
